...
class TasksController {
  async getTasks(req: any, res: any) {
    if(!req.user.id_fire) return res.status(401).json('Você precisa logar');
  ...
  async addTasks(req: any, res: any) {
    if(!req.user.id_fire) return res.status(401).json('Você precisa logar');
    const response = await TaskCreater.createTask(req.body, req.user.id_fire);
    res.json(response)
  }
  ...
  async deleteTask(req: any, res: any) {
    try {
      if(!req.user.id_fire) return res.status(401).json('Você precisa logar');
      const response = await TaskDeleter.deleteTask(req.params.id, req.user.id_fire);
      res.json(response);
    } catch (error: any) {
  ...
  async editTask(req: any, res: any) {
    try {
      if(!req.user.id_fire) return res.status(401).json('Você precisa logar');
      const response = await TaskEditer.editTask(req.body, req.user.id_fire);
      res.json(response);
    } catch (error: any) {
      res.status(400).json(error?.message);
    }
